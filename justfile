...
tmp_dir := "examples"
script_dir := "python-template-script"
...
render-script:
    rm -rf {{ tmp_dir }}/{{ script_dir }}
    {{ copier_base }} {{ tmp_dir }}/{{ script_dir }} {{ copier_flags }} --data project_slug=demo-script --data package_mode=no-package

smoke: render-flat render-hex render-script

clean:
    rm -rf {{ tmp_dir }}/{{ flat_dir }} {{ tmp_dir }}/{{ hexagonal_dir }} {{ tmp_dir }}/{{ script_dir }}
